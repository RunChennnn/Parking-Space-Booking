...
function BookingStub (props) {
  const cardStyle = {
    ...
    margin: '20px 50px',
    padding: '15px',
    backgroundColor: props.currentUserID === props.ownerID ? '#ffefa8' : '#f8f8f8',
    borderColor: props.currentUserID === props.ownerID ? '#f7ae02' : '#aaa',
    borderStyle: 'solid',
    borderWidth: props.currentUserID === props.ownerID ? '2px' : '1px',
    borderRadius: '10px',
    display: 'grid',
  ...
  return (
    <Card key={props.id} style={cardStyle}>
      <div>
        ...
        <Typography variant='h6'>{address} {props.currentUserID === props.ownerID && '(as owner)'}</Typography>
